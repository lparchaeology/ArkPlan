...
class SnappingWidget(QGroupBox, Ui_SnappingWidget):
    ...
    def loadProject(self, plugin):
        self._setLayer(plugin.iface, plugin.plan.pointsBuffer, self.snapBufferPointsTool)
        self._setLayer(plugin.iface, plugin.plan.linesBuffer, self.snapBufferLinesTool)
        self._setLayer(plugin.iface, plugin.plan.polygonsBuffer, self.snapBufferPolygonsTool)
        self._setLayer(plugin.iface, plugin.plan.pointsLayer, self.snapPlanPointsTool)
        self._setLayer(plugin.iface, plugin.plan.linesLayer, self.snapPlanLinesTool)
        self._setLayer(plugin.iface, plugin.plan.polygonsLayer, self.snapPlanPolygonsTool)
        self._setLayer(plugin.iface, plugin.site.pointsLayer, self.snapSitePointsTool)
        self._setLayer(plugin.iface, plugin.site.linesLayer, self.snapSiteLinesTool)
        self._setLayer(plugin.iface, plugin.site.polygonsLayer, self.snapSitePolygonsTool)
        Snapping.setSnappingMode(Snapping.SelectedLayers)
        Snapping.setIntersectionSnapping(True)
        Snapping.setTopologicalEditing(True)
        Snapping.setLayerSnappingEnabled(plugin.plan.linesBuffer.id(), True)
        Snapping.setLayerSnappingEnabled(plugin.plan.polygonsBuffer.id(), True)
        Snapping.setLayerSnappingEnabled(plugin.grid.pointsLayer.id(), False)
        Snapping.setLayerSnappingEnabled(plugin.grid.linesLayer.id(), False)
        Snapping.setLayerSnappingEnabled(plugin.grid.polygonsLayer.id(), False)
