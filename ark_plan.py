...
class ArkPlan:
    ...
    def getLayerByName(self, dir, name, styleDir, styleName, groupIndex):
    ...
    def initialise(self):
        ...
        if (self.gridLayer is None):
            self.gridLayer = self.getLayerByName(self.contextLayerDir, self.gridLayerName, self.stylesDir, self.gridStyleName, self.layerGroupIndex)
        if (self.schematicLayer is None):
            self.schematicLayer = self.getLayerByName(self.contextLayerDir, self.schematicLayerName, self.stylesDir, self.schematicStyleName, self.layerGroupIndex)
        if (self.polygonsLayer is None):
            self.polygonsLayer = self.getLayerByName(self.contextLayerDir, self.polygonsLayerName, self.stylesDir, self.polygonsStyleName, self.layerGroupIndex)
        if (self.linesLayer is None):
            self.linesLayer = self.getLayerByName(self.contextLayerDir, self.polygonsLayerName, self.stylesDir, self.linesStyleName, self.layerGroupIndex)
        if (self.levelsLayer is None):
            self.levelsLayer = self.getLayerByName(self.contextLayerDir, self.levelsLayerName, self.stylesDir, self.levelsStyleName, self.layerGroupIndex)

        # Setup the in-memory buffer layers
        self.bufferLegendGroup = self.iface.legendInterface().addGroup(self.bufferGroupName)
    ...
    def georeferencePlan(self, rawFile):
        georefDialog = ArkGeorefDialog(rawFile, self.geoDir, self.crs, self.gridLayerName, self.gridLayerX, self.gridLayerY)
