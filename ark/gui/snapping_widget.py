...
class SnappingWidget(QGroupBox, Ui_SnappingWidget):
    ...
    def loadProject(self, plugin):
        ...
        self._setLayer(plugin.iface, plugin.plan.layer('points'), self.snapPlanPointsTool)
        self._setLayer(plugin.iface, plugin.plan.layer('lines'), self.snapPlanLinesTool)
        self._setLayer(plugin.iface, plugin.plan.layer('polygons'), self.snapPlanPolygonsTool)
        self._setLayer(plugin.iface, plugin.site.layer('points'), self.snapSitePointsTool)
        self._setLayer(plugin.iface, plugin.site.layer('lines'), self.snapSiteLinesTool)
        self._setLayer(plugin.iface, plugin.site.layer('polygons'), self.snapSitePolygonsTool)
        Snapping.setSnappingMode(Snapping.SelectedLayers)
        Snapping.setIntersectionSnapping(True)
        Snapping.setTopologicalEditing(True)
        Snapping.setLayerSnappingEnabled(plugin.plan.buffer('lines').id(), True)
        Snapping.setLayerSnappingEnabled(plugin.plan.buffer('polygons').id(), True)
        Snapping.setLayerSnappingEnabled(plugin.grid.layer('points').id(), False)
        Snapping.setLayerSnappingEnabled(plugin.grid.layer('lines').id(), False)
        Snapping.setLayerSnappingEnabled(plugin.grid.layer('polygons').id(), False)
